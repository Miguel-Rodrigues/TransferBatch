...
public class TransferBatchService
{
    ...
    {
        using var _fileStream = new FileStream(options.File, FileMode.Open, FileAccess.Read);
    ...
    private async Task CheckProgress(Task monitor, bool verbose)
    {
        ...
        do
        {
            ...
            if (verbose)
            {
                Console.WriteLine($"Progress: {progress:F2}%, Elapsed {(int)sw.Elapsed.TotalSeconds}s, {_totalLinesRead} lines written");
            }
    ...
    /// </summary>
    /// <returns>The asynchronous task of this call</returns>
    private async Task OutputComissions()
    {
